...
| `atelet.snapshot.size` | atelet | histogram | on-disk size in bytes of each snapshot image written during checkpoint — the blocks the filesystem allocated, not the length the file reports. The two agree for the gVisor images, which are dense, and differ sharply for the micro-VM memory image, which is sparse: its length is a constant, the guest's configured RAM (labels `file.name`, `ate.template.namespace`, `ate.template.name`) |
